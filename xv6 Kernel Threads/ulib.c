...
// kThread library implementations
	...
	int create_kthread(void (*fnc)(void *), void *arg)
	{
		...
		// return PID to the parent and 0 to the child (if successful), -1 otherwise.

	int join_kthread(void)
	{
	    return join();
	}
		// returns the waited-for PID (when successful), -1 otherwise.
