...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  // Clone system call code in proc.c to create a new kernel thread 
  ...
  // while a thread shares the parent's address space and
  // in xv6, the address space is recorded in the process 
  // structure with the pgdir (page directory) field.
  // To share the parent's address space with the clone we can
  // simply do this
  thread->pgdir = curproc->pgdir;

  //saving important information from the parent process to the clone
  ...
  *thread->tf = *curproc->tf;
  /*
  Saving the trap frame for the current system call
  The trap frame stores the state of the processes' registers,
  and will be restored by the OS on the return-from-trap
*/
  ...
  // they will be the same value because we are returning into
  // a function
...
// join system call code in proc.c to wait for a thread to exite
...
int join(void)
{
  ...
  for(;;) // running forever for loop
  {
     ...
     // Check if thread "t" is a child thread
  ...
  // Clild thread found
       haveThreads = 1;
       if(t->state == ZOMBIE) 
       {  // Don't free pgdir -adress space- of the child thread [freevm(t->pgdir);]
        // or you will kill both of the thread and its parent 
      pid = t->pid;
      kfree(t->kstack);
      t->kstack = 0;
      t->pid = 0;
